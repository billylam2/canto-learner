...
- **`DUB_SYNC_GCS_BUCKET`** in `.env.local` — the name of a Google Cloud Storage bucket (e.g. created via `gsutil mb gs://<bucket-name>` or the console) that the auto-mark pipeline can read and write. Speech-to-Text requires audio longer than its inline-request limit to be passed as a `gs://` URI rather than embedded directly, so each video's downloaded audio is uploaded there temporarily and deleted again once transcription finishes — never kept or served.
