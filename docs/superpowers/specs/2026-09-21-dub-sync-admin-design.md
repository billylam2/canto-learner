...
- **Pairing strategy**: if the two sides produce the same number of speaker turns (within their respective content anchors), pair them 1:1 by order — both `cantoStart`/`cantoEnd` and `englishStart`/`englishEnd` come from real transcribed timestamps. If the counts differ, fall back to proportional stretch and surface a warning in the auto-mark response (shown in the admin page) so it's clear that episode's timing is approximate rather than silently accepting lower-quality output. The fallback prefers English's turn boundaries as the reference (mapped onto Cantonese via `cantoTimeFor`, the inverse of `englishTimeFor`) rather than Cantonese's own turns, discovered during live QA to be necessary: Google Cloud Speech-to-Text does not support speaker diarization for Cantonese (`yue-Hant-HK`) at all — in any model, v1 or v2/Chirp — so it silently tags every word with the same speaker rather than erroring, making Cantonese's own turns essentially always a single undiarized blob spanning the whole clip. English (`en-US`) diarizes well, so using it as the reference produces real per-line segments instead of one giant one. Only when English also has no usable turns does the fallback fall further back to Cantonese's own (likely degenerate) turns.
...
Filters `cantoTurns` to `[cantoContentStart, cantoContentEnd]` and `englishTurns` to `[englishContentStart, englishContentEnd]`. If both filtered lists are non-empty and the same length, zips them index-by-index — `englishStart`/`englishEnd` come directly from the matching English turn, not from `englishTimeFor` — and returns `usedFallback: false`. Otherwise, if `englishTurns` has any turns within its content anchors, falls back to `englishCuesToCandidateSegments(englishTurns, anchors)` (english turns as the reference, mapped onto canto via `cantoTimeFor`); only when english has none either does it fall back further to `cuesToCandidateSegments(cantoTurns, anchors)` (canto turns as the reference, mapped onto english via `englishTimeFor`). Either fallback returns `usedFallback: true`.
